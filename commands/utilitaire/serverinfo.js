	...
	run: async (client, message, args, prefix, color) => {
		...
		if (client.config.owner.includes(message.author.id) || db.get(`ownermd_${client.user.id}_${message.author.id}`) === true || perm || db.get(`channelpublic_${message.guild.id}_${message.channel.id}`) === true) {
			...
			const ServerInfo = new Discord.MessageEmbed()
				...
				.addField(`Nombre de membres en ligne :`, `__${guild.members.cache.filter(m => m.presence?.status === 'online' || m.presence?.status === 'dnd' || m.presence?.status === 'streaming' || m.presence?.status === 'idle').size}__`, true)
				.addField(`Nombre d'humains :`, `__${guild.members.cache.filter((m) => !m.user.bot).size}__`, true)
				.addField(`Nombre de bots :`, `__${guild.members.cache.filter((m) => m.user.bot).size}__`, true)
				.addField(`Nombre d'utilisateurs en vocal :`, `__${guild.members.cache.filter(m => m.voice.channel).size}__`, true)
				.addField(`Nombre d'utilisateurs sans rôle :x: :`, `__${NoRoles}__`, true)
				.addField(`Nombre de boosts :`, `__${guild.premiumSubscriptionCount}__`, true)
				//.addField(`Nombre De Booster(s) <a:4_nitroboost:1217195480052666378> :`, `${boosters}`, true)
				.addField(`Nombre de rôles:`, `__${guild.roles.cache.size}__`, true)
				.addField(`Nombre de salons :`, `__${guild.channels.cache.size}__`, true)
				.addField(`Nombre d'émojis total :`, `__${guild.emojis.cache.size}__`, true)
